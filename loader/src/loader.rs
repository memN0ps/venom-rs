use std::{arch::asm, ffi::CStr, collections::BTreeMap};

use winapi::{um::winnt::{PIMAGE_DOS_HEADER, IMAGE_DOS_SIGNATURE, PIMAGE_NT_HEADERS, PIMAGE_SECTION_HEADER, PIMAGE_NT_HEADERS64, IMAGE_DIRECTORY_ENTRY_EXPORT, PIMAGE_EXPORT_DIRECTORY, IMAGE_NT_SIGNATURE}, ctypes::c_void};
...
pub fn reflective_loader() {
    ...
    // get exports by hash
    let loadliba_hash = get_exports_by_hash(kernel32_base as _, LOADLIBRARYA_HASH).expect("failed to get exports by hash");
...
fn get_exports_by_hash(module_base: *mut c_void, hash: u32) -> Option<*mut u8> {
...
/// gets the image export directory table
unsafe fn get_module_exports(module_base: *mut c_void) -> BTreeMap<String, usize> {
    ...
    #[cfg(target_arch = "x86")]
    let nt_header = (module_base as usize + (*dos_header).e_lfanew as usize) as PIMAGE_NT_HEADERS;
